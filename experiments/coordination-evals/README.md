...
## Results
See [`results.md`](./results.md) — regenerated by `run.ts`. Tier-A headline (deterministic): **CTSR 1/4 → 4/4**, merge conflicts **1 → 0**, wasted LOC **5 → 0**, routing **100% hit / 0% false** (unrelated services correctly not warned), independent control **unchanged (no harm)**. Tier-B numbers (outcome lift + the lost-in-the-middle curve) are appended after a live run.
