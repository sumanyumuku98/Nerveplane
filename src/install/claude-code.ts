...
7. **Memory** — durable and shared across agents and CLIs. At the start of a task, call \`memory\` with action='recall' for prior context (relevant memories are also injected at session start). As you work, use action='remember' for durable gotchas/decisions (kind='fact'); before finishing or handing off, remember your progress and where you left off (kind='episode') so another agent — even a different CLI — can resume. Scope with repo_id/task_id.
