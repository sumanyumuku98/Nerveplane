...
6. **Memory** — durable, shared across agents and CLIs. At the **start of a task**, call \`memory\` \`action='recall'\` for prior context. As you work, \`action='remember'\` durable gotchas/decisions (\`kind='fact'\`) and, before finishing or handing off, your progress and where you left off (\`kind='episode'\`) — so another agent (even a different CLI) can resume. Scope with \`repo_id\`/\`task_id\`. If you finish a turn having recorded a decision, handed off a task, or made substantial changes without saving anything, Nerveplane reminds you at turn-end — but write proactively rather than waiting for the nudge.
