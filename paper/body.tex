...
\emph{scheduling}: predict what each agent will touch, partition the work into disjoint scopes,
and order merges along the producer$\rightarrow$consumer dependency graph, all up front. We build
this planner into \sysname, a local-first coordination plane for coding agents, and evaluate it
with NP-Bench, a three-arm evaluation suite (no coordination; reactive detection; proactive
planning) that runs both as a deterministic simulation and with live agents. Deterministically, the
...
change, the planner rescues an outcome that both no-coordination and reactive detection miss on
every seed (0.00/0.00$\rightarrow$1.00 on a frontier model; 0.00/0.00$\rightarrow$0.60 on a small
one), and agents respect their assigned scopes (0/5 leakage). A separate memory result collapses the
repeated-mistake rate from 1.00 to 0.00 on both a strong and a weak model: a decision recorded in
one session stops a later, differently sessioned agent from repeating it, an information effect no
...
contexts; its value is cost and capacity, not attention. The coordination win is structural. It
lives in work allocation, not model reasoning, so it survives stronger models rather than being
erased by them.
\end{abstract}
...
scheduling problem}. Before the agents start, predict the \emph{scope} each will touch, partition
...
objection is the research question in disguise. A schedule helps only if (a) scopes can actually be
\emph{predicted} before the work runs, and (b) the agents actually \emph{respect} the scopes they
are handed. Neither is guaranteed; an agent told to stay out of a file may edit it anyway. Both are
measurable, and that measurement is the contribution, not the tautology.
...
(\Cref{sec:planner}).
\item \textbf{An evaluation suite.} NP-Bench scores three arms (no coordination \arm{C0}, reactive
detection \arm{C1-detect}, proactive planning \arm{C1-plan}) both deterministically and with live
agents from two model families, on the coordination axis that single-agent benchmarks like SWE-bench
do not measure (\Cref{sec:npbench}). We present it as a focused study of \emph{when} coordination
helps, not a comprehensive benchmark.
\item \textbf{Results with an honest capability analysis.} The planner strictly dominates both
baselines; a vague reactive warning is worth about as much as nothing to a live agent; and we
separate the part of the effect that is capability-independent from the part that is not
...
\emph{predicted code scopes} of LLM agents, where the novel question is empirical: can the scopes
be predicted, and will the agents respect them?
...
cross-session and cross-agent: a decision recorded by one session prevents a later, differently
sessioned agent from repeating a mistake, and we isolate it as an information effect that holds
...
coordination plane for coding agents: a single per-machine daemon, a SQLite store, and a set of
...
\paragraph{Model.} Let $W$ be a set of work items. Each item $w$ has a \emph{predicted scope}
...
\paragraph{Claim (assumption-scoped).} If the predicted scopes are accurate and the dependency
...
because it acts only after a collision has been sensed. The claim's premises are exactly the
empirical unknowns (scope-prediction accuracy and agent scope-compliance), which we measure in
\Cref{sec:results}.
...
is the proactive planner: each agent receives a disjoint scope and a merge slot before it starts.
...
planner arm. The signal is capability- and vendor-independent (all three models fail at 0.00 without
it); acting on the plan is partly capability-dependent (the two frontier models $1.00$, the small
model $0.60$). On the frontier model the planner and baselines separate with non-overlapping
...
explicit up-front assignment carries the contract shape the agent needs. This is the
detector-to-planner thesis in one line. Third, the failure of \arm{C0} is not a capability failure:
...
gotcha and records it, a later, differently-sessioned agent should stop repeating the mistake---but
...
The live contract result has a two-layer structure worth naming. The coordination \emph{signal} is
capability-independent: without it, a frontier model, a small model, and a second vendor's model all
fail at 0.00, because the fact is absent from their context, not beyond their reasoning.
...
split matters for anyone worried that better models will make coordination unnecessary. The
information half will not be closed by scale; the execution half already benefits from it.

It also explains when reactive detection is enough and when it is not. When dependencies are
sequential and agents are slow relative to sensing, a warning lands in time and the detector matches
the planner. When work is concurrent, agents are fast, or $N$ is large, the warning is structurally
too late, and only scheduling helps. The deterministic sweep makes this concrete: the detector
tracks no-coordination exactly under contention, and the gap to the planner grows with $N$.

For the broader question of \emph{verifying} multi-agent systems, NP-Bench is an environment-grounded
evaluator: it runs real agents in a real git integration and reads verifiable signals---clean-merge
success, scope compliance, contract adaptation, cross-session decision adherence---directly off the
merged tree, rather than trusting each agent's self-report. Coordination failures are invisible to
single-agent evaluation: each agent, judged alone, did fine; the system still broke on integration.
A three-arm design (nothing / reactive / proactive) turns ``did the coordination layer help'' into a
measurement rather than an assertion, and the honest negative arm keeps it from being a demo dressed
as a result.
...
The most important limitation is scope prediction. In these experiments the predicted scopes are
\emph{hand-seeded} in the harness (each item's declared files and consumed contracts), not inferred
...
Parallel coding agents fail at the seams, and the seams are a scheduling problem. Predict what each
agent will touch and schedule around it---disjoint scopes for concurrent work, producers merged
...
scales---its value is cost and capacity. The throughline is that the coordination win is structural:
it lives in how work is allocated, so stronger models sharpen it rather than erase it. The open
...
open source.\fi{} Tier A is deterministic and CI-gated; Tier B is a set of live-agent runs that are
