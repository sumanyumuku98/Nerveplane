...
integrated system can still be broken, and only a real merge-and-test reveals it. Across the two capability tiers and two vendors we tested, the coordination win is
structural: it lives in work allocation, not model reasoning, so it is not erased as models get
stronger.
\end{abstract}
...
objection is the research question in disguise. A schedule helps only if (a) scopes can be
\emph{predicted} before the work runs and (b) the agents actually \emph{respect} the scopes they
are handed. In this paper the scopes are \emph{declared}---given by each work item's stated files
and consumed contracts---and we measure (b): an agent told to stay out of a file may edit it anyway,
and whether it complies is exactly what the live experiments check. Automatic scope prediction (a)
is an open problem we flag but do not solve. The contribution is the measurement, not the tautology.
...
The system and the NP-Bench harness are open source (\sysname v0.17.0,
\url{https://github.com/sumanyumuku98/Nerveplane}).\fi} A routing engine then decides who
...
it lives in how work is allocated, so---across the models we tested---stronger models sharpen it rather than erase it. The open
